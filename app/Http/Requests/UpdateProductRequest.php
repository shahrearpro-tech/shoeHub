...
class UpdateProductRequest extends FormRequest
{
    ...
    {
        $productId = $this->route('id'); // Get ID from route parameter

        return [
            // Basic Information
            'name' => 'required|string|max:200',
            'slug' => 'nullable|string|max:200|unique:products,slug,' . $productId,
            ...
            'gallery.*.image' => 'required_with:gallery|image|mimes:jpeg,jpg,png,webp|max:2048',
            'gallery.*.color' => 'nullable|string|max:50',
            
            // Existing Image Colors (update only)
