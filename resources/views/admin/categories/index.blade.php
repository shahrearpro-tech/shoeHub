...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
    ...
    <div class="lg:col-span-2 bg-white rounded-[40px] shadow-2xl border border-white/20 overflow-hidden animate-slide-in-left">
        <div class="px-10 py-8 border-b border-gray-50 bg-white flex justify-between items-center">
            <div>
                <h2 class="text-3xl font-black text-secondary tracking-tight">Categories</h2>
                <p class="text-gray-400 text-xs font-bold uppercase tracking-widest mt-1">Organize your catalog</p>
            </div>
        </div>
        <div class="overflow-x-auto">
            <table class="w-full text-left border-collapse">
                ...
                <tbody class="divide-y divide-gray-50">
                    ...
                    <tr class="group hover:bg-gray-50/30 transition-colors">
                        <td class="px-10 py-6">
                            <div class="flex items-center gap-4">
                                ...
                                <span class="font-bold text-secondary text-sm">{{ $category->name }}</span>
                            </div>
                        </td>
                        <td class="px-10 py-6 font-medium text-sm text-gray-500">
                             <span class="px-3 py-1 bg-gray-100 rounded-lg text-xs font-bold text-gray-600">{{ $category->products_count }}</span>
                        </td>
                        <td class="px-10 py-6">
                            <span class="px-3 py-1 rounded-lg text-[10px] font-black uppercase tracking-wider {{ $category->status === 'active' ? 'bg-green-100 text-green-600' : 'bg-red-100 text-red-600' }}">
                                {{ $category->status }}
                            </span>
                        </td>
                        <td class="px-10 py-6 text-right">
                            <div class="flex items-center justify-end gap-3 opacity-0 group-hover:opacity-100 transition-opacity">
                                <a href="{{ route('admin.categories.edit', $category->id) }}" class="w-8 h-8 rounded-full bg-blue-50 text-blue-600 flex items-center justify-center hover:bg-blue-600 hover:text-white transition-all shadow-sm">
                    ...
                    @empty
                    <tr>
                        <td colspan="4" class="px-10 py-12 text-center text-gray-400 font-medium">No categories found</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
    ...
    <div class="lg:col-span-1">
        <div class="bg-white rounded-[40px] shadow-2xl border border-white/20 p-10 sticky top-8 animate-slide-in-right">
            <h3 class="text-xl font-black text-secondary tracking-tight mb-2">Add Category</h3>
            <p class="text-gray-400 text-xs font-bold uppercase tracking-widest mb-8">Create new classification</p>
            
            <form action="{{ route('admin.categories.store') }}" method="POST" class="space-y-6">
                @csrf
                <div>
                    <label class="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2 px-1">Name</label>
                    <input type="text" name="name" required class="w-full px-6 py-4 rounded-2xl bg-gray-50 border-none text-secondary font-bold focus:ring-2 focus:ring-orange-500/20 outline-none transition" placeholder="e.g. Sneakers">
                </div>
                ...
                <div>
                    ...
                    <textarea name="description" rows="3" class="w-full px-6 py-4 rounded-3xl bg-gray-50 border-none text-secondary font-medium focus:ring-2 focus:ring-orange-500/20 outline-none transition" placeholder="Optional description..."></textarea>
                </div>
